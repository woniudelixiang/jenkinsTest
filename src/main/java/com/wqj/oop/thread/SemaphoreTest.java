	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10; i++) {
			Runnable runnable = new Runnable() {
				...
				public void run() {
					try {
						sp.acquire(); // 获取信号量 ，如果信号量内部计数器大于0 ，信号量减1 ，如果信号量的计数器等于0 ，将阻塞等待， 
					...
					sp.release(); // 释放信号量 ，别忘了在finally中使用，注意：多次调用该方法，会使信号量的许可数增加，达到动态扩展的效果，如：初始permits为1， 调用了两次release，最大许可会改变为2
