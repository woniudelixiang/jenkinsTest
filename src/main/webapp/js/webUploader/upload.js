...
//             runtimeOrder: 'flash',
        ...
        function addFile( file ) {
            var $li = $( '<li id="' + file.id + '">' +
             ...
             console.log("file.getStatus(): " + file.getStatus());
            ...
            if ( file.getStatus() === 'invalid' ) {
               ...
               // 生成缩略图
                uploader.makeThumb( file, function( error, src ) { // 如果生成缩略图有错误，此error将为真   第二个为src, 缩略图的URL值
                    var img;
                    if ( error ) {
                        $wrap.text( '不能预览' );
                        return;
...
//                        });
        ...
        uploader.onFileQueued = function( file ) {
            ...
            console.log("当文件被加入队列以后触发" +(fileCount === 1));
            if ( fileCount === 1 ) {
                $placeHolder.addClass( 'element-invisible' );
                $statusBar.show();
            }
